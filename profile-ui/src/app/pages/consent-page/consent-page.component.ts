...
export class ConsentPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      this.auth$ = this.http.get<any>(window.location.origin + "/profile/api/auth/consent?consent_challenge=" + this.consent_challenge).pipe(
        map((data: any) => {
          console.log(data)
  ...
  postConsent(value:string): void {
    this.http.post(window.location.origin + "/profile/api/auth/consent?consent_challenge=" + this.consent_challenge + "&_csrf=" + this.csrf, {
      _csrf: this.csrf,
